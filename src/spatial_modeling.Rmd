...
crime_data <- st_read("../data/final_dataset/extracted") #Reading the main data frame


# adding crime count to the dataset and manipulating data
data_additional <- st_read("../../data/QOL/Final_dataset.shp")
data_additional <- data_additional %>%
  dplyr::select(id, propcrimct, area_acre)
geometry_factored <- st_geometry(crime_data)
st_geometry(crime_data) <- NULL
# Check if data_additional has geometry
if ("geometry" %in% names(data_additional)) {
  # Remove geometry
  st_geometry(data_additional) <- NULL
}
data_additional <- data_additional %>%
  dplyr::select(id, propcrimct, area_acre)
data_combined <- crime_data %>%
  left_join(data_additional, by = "id")
crime_data <- st_as_sf(data_combined, geometry = geometry_factored)
names(crime_data)[names(crime_data) == "propcrimct"] <- "crime.count"
names(crime_data)[names(crime_data) == "property.c"] <- "crime.rate"



#Extracting the coordinates and creating queens spatial matrix
coords_crime_data <- st_coordinates(st_centroid(crime_data))
crime_data$X <- coords_crime_data[, "X"]
crime_data$Y <- coords_crime_data[, "Y"]

data_nb <- poly2nb(crime_data, queen = TRUE)
adj_matrix <- nb2mat(data_nb, style="B")
row.names(adj_matrix) <- NULL
data_listw <- nb2listw(data_nb, style = "W")

#Log transformation of network measures
crime_data$average.be_log <- log(crime_data$average.be + 1)
crime_data$intersecti_log <- log(crime_data$intersecti+ 1)
crime_data$average.cl_log <- log(crime_data$average.cl + 1)
crime_data$average.sh_log <- log(crime_data$average.sh + 1)

# Defining unique identifier for each feature
crime_data$UID <- as.numeric(factor(crime_data$id))

# Create spatial lag of the dependent variable (crime.count)
crime_data$spatial_lag <- lag.listw(data_listw, crime_data$crime.count)

#Removing unnecessary object
rm(data_additional)
rm(data_combined)
rm(geometry_factored)

#Removing unneccessary Variables
crime_data <- crime_data %>%
  dplyr::select(-average.be,-intersecti, -average.cl, -average.sh)
...
dependent_var_name <- "crime.count"

street_measures <- c("average.be_log", "intersecti_log", "average.cl_log", "average.sh_log")

# Prepare the model formula (with inclusion of the offset parameter)
formula <- crime.count ~ average.be_log + intersecti_log + average.cl_log + 
           average.sh_log + Factor_1 + Factor_2 + Factor_3 + Factor_4 + 
           Factor_5 + Factor_6 + Factor_7 + Factor_8 + Factor_9 + offset(log(area_acre))

crime_data$area_acre_logged <- log(crime_data$area_acre)

formula.no.offset <- crime.count ~ average.be_log + intersecti_log + average.cl_log + 
           average.sh_log + Factor_1 + Factor_2 + Factor_3 + Factor_4 + 
           Factor_5 + Factor_6 + Factor_7 + Factor_8 + Factor_9

```

#Descriptive stats of Street network measures

```{r Descriptive stats,include=FALSE}

crime_data_df <- st_drop_geometry(crime_data)


desc_stats <- psych::describe(crime_data_df[, street_measures])
#View(desc_stats)
...
#Checking if "crime.count" follows Poisson dist or BN dist.
var <- crime_data$crime.count
...
```{r GLM NB-based, echo = TRUE}
nb.glm <- glm.nb(formula, data = crime_data);summary(nb.glm)
...
moran_test_dependent <- moran.test(crime_data$crime.count, data_listw);print(moran_test_dependent) # Moran's test on dependent variable
...
m0.besag  <- inla(formula.m0.besag,
            family = "nbinomial",
            data = crime_data,
            offset = crime_data$area_acre_logged,
...
f1 <- crime.count ~ average.be_log + intersecti_log + average.cl_log + 
           average.sh_log + Factor_1 + Factor_2 + Factor_3 + Factor_4 + 
           Factor_5 + Factor_6 + Factor_7 + Factor_8 + Factor_9
mmatrix <- model.matrix(f1, crime_data)
...
# Standardizing the variables in crime_data
crime_data_std <- crime_data  # Copy the original data

# Apply z-score standardization (excluding the offset term)
crime_data_std$Factor_1 <- scale(crime_data_std$Factor_1)
crime_data_std$Factor_2 <- scale(crime_data_std$Factor_2)
crime_data_std$Factor_3 <- scale(crime_data_std$Factor_3)
crime_data_std$Factor_4 <- scale(crime_data_std$Factor_4)
crime_data_std$Factor_1 <- scale(crime_data_std$Factor_1)
crime_data_std$Factor_5 <- scale(crime_data_std$Factor_5)
crime_data_std$Factor_6 <- scale(crime_data_std$Factor_6)
crime_data_std$Factor_7 <- scale(crime_data_std$Factor_7)
crime_data_std$Factor_8 <- scale(crime_data_std$Factor_8)
crime_data_std$Factor_9 <- scale(crime_data_std$Factor_9)
crime_data_std$average.be_log <- scale(crime_data_std$average.be_log)
crime_data_std$intersecti_log <- scale(crime_data_std$intersecti_log)
crime_data_std$average.cl_log <- scale(crime_data_std$average.cl_log)
crime_data_std$average.sh_log <- scale(crime_data_std$average.sh_log)
crime_data_std$crime.count <- scale(crime_data_std$crime.count)
...
# bandwidth <- ggwr.sel(formula, data = crime_data_std,
#                       #adapt = TRUE,
#                       coords = coords_crime_data, family = negative.binomial(2.702), verbose = TRUE)

# # Perform the Geographically Weighted Poisson Regression
# gwr_model <- ggwr(formula,
#                   data = crime_data_std,
#                   coords = coords_crime_data,
...
crime_data_temp <- as(crime_data, "Spatial")

gwr_model_local <- gw.cov(crime_data_temp,vars = independent_vars,bw = 33776.24, cor = FALSE)
...
for (i in 1:length(predictor_names)) {
  ...
  #df <- nrow(crime_data) - length(street_measures) - 1  
...
crime_data_final <- cbind(crime_data, gwr_model_local$SDF@data)
...
for (var in street_measures) {
  ...
  df <- nrow(crime_data) - length(street_measures) - 1  # Adjusting for predictors & intercept
  
  # Compute Standard Error (SE) for the local coefficient
  se <- sd_dev_resid / sqrt(nrow(crime_data))
...
crime_data_final <- cbind(crime_data, final_results_df)
View(crime_data_final)
#st_write(crime_data_final, "../GWR_NB_in_R.shp", delete_dsn = TRUE)
...
y_mean <- mean(crime_data$crime.count)
TSS <- sum((crime_data$crime.count - y_mean)^2)
...
# crime_data_no_geometry <- st_drop_geometry(crime_data)
# 
# # Fit the negative binomial spatial GLM
# model.CAR.nb <- glmmfields(formula.no.offset,
#                     data = crime_data_no_geometry,
#                     lat = "Y",
#                     lon = "X",
#                     family = nbinom2(link = "log"),
#                     offset = crime_data_no_geometry$area_acre_logged,
#                     verbose = TRUE
#                     );model.CAR.nb
# ```
...
# poisson.glm <- glm(formula, family = poisson, data = crime_data);summary(poisson.glm)
# 
# # capturing the R-squared values
...
# print(fit)
# print(plot(fit))
# print(sp_diag(y = fit, shape = crime_data))
# 
# ```
...
# model_inla <- CAR_INLA(carto = crime_data, ID.area = "id", O = "crime.count", E = "area_acre_logged", X = independent_vars, model = "global", n.sample = 10000)
# 
...
# 
# DM <- gw.dist(dp.locat = coords_crime_data)
# bandwidth <- bw.ggwr(formula,data = crime_data, dMat =DM,family="poisson")
# 
# gwpr_model <- ggwr.basic(formula, data = crime_data,dMat = DM, bw = bandwidth, family = "poisson")
# 
# gwpr_model
# 
# # Deviance
