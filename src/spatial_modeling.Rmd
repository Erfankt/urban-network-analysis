...
crime_column <- "propcrimct"                                  # Specify the crime variable used in the analysis
...
data_additional <- st_read(here("data/neighborhoods/neighborhoods.shp")) |>
  ...
  st_drop_geometry() |>
  select(id, all_of(crime_column), area)
...
coords <- st_coordinates(st_centroid(original_data))
original_data$X <- coords[,1]
original_data$Y <- coords[,2]

nb <- poly2nb(original_data, queen = TRUE)
lw <- nb2listw(nb, style = "W")

# 4. Log-transform network PCs
original_data <- original_data |>
  mutate(
    across(matches("^PC[0-9]+_net$"), ~ log(.x + 1), .names = "{.col}_log")
  ) |>
  select(-matches("^PC[0-9]+_net$"))

# 5. Spatial lag of crime
original_data$spatial_lag <- lag.listw(lw, original_data[[crime_column]])

# 6. Model formulas
independent_vars <- c(
  grep("_log$", names(original_data), value = TRUE),
  grep("^PC[0-9]+_ctrl$", names(original_data), value = TRUE)
)

formula_string <- paste(crime_column, "~", paste(independent_vars, collapse = " + "))
...
var <- original_data[[crime_column]]
...
nb.glm <- glm.nb(base.model, data = original_data); summary(nb.glm)# NB GLM
print(rsq(nb.glm, adj = TRUE))                                     # R²
check_overdispersion(nb.glm)                                       # Overdispersion

# Diagnostics
res.nb <- residuals(nb.glm, type = "pearson")
print(moran.test(original_data[[crime_column]], lw))               # Moran on dependent
...
original_data$UID <- 1:nrow(original_data)

# 2. Build adjacency graph in INLA format
nb <- poly2nb(original_data)
...
# -----------------------------
crime_var <- crime_column      # = "propcrimct"
W_listw   <- lw                # spatial weights object
area_log  <- log(original_data$area)   # earlier you used offset(log(area))

# Ensure UID exists
if (!"UID" %in% names(original_data)) {
  original_data$UID <- seq_len(nrow(original_data))
}
